...
template <typename T>
Node<T>* Node<T>::locate(T val)
{
    return val > value ?
        (rightPointer == nullptr ? nullptr :
            rightPointer->value == val ? rightPointer : rightPointer->locate(val)) :
        (leftPointer  == nullptr ? nullptr :
            leftPointer->value == val ? leftPointer : leftPointer->locate(val));
}
...
void Node<T>::shrink(Node<T>* root)
{
    ...
    while (upperBranch->leftPointer && upperBranch->leftPointer->value != value && upperBranch->rightPointer && upperBranch->rightPointer->value != value)
    {
    ...
    if (leftPointer)
    {
        ...
        replacementLocation->shrink(root);
...
void BSTree<T>::remove (T val)
{
    if (rootPointer)
    {
        if (rootPointer->value == val && !rootPointer->leftPointer && !rootPointer->rightPointer)
        {
            delete rootPointer;
            rootPointer = nullptr;
            numberOfNodes -= 1;
        ...
        {
            Node<T>* target = rootPointer->locate(val);
            if (target)
            {
